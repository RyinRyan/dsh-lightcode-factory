...
节点状态是 `pending | running | completed | cancelled | failed`。同一 run mutation 串行；每次先读取 revision、计算新聚合，再 CAS 保存。取消先保存 cancelled 后清理 timer/abort，晚到 timer、output 或 observation 不能覆盖终态。带 `scheduledFor` 的 queued 在相同 workflow id/version 注册后恢复计时；已过期计划立即进入可运行队列。running 和无计划时间的遗留 queued 在 Host 重启后标记 failed，不伪造 checkpoint 恢复。
