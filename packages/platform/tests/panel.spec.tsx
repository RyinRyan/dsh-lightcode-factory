...
import type { WorkflowPlatformClientSnapshot } from 'lightcode-factory-backend/client'
import { WorkflowPlatformPanel, type WorkflowPlatformPanelProps } from '../src/client/WorkflowPlatformPanel.tsx'
import { zh, type WorkflowPlatformKey } from '../src/client/locales.ts'
...
function props(snapshot: WorkflowPlatformClientSnapshot, start = vi.fn().mockResolvedValue({})) {
  return {
    useWorkflowSnapshot: (select: (value: WorkflowPlatformClientSnapshot) => unknown) => select(snapshot),
    start, cancel: vi.fn(), review: vi.fn(), refresh: vi.fn(),
    t: (key: WorkflowPlatformKey, values?: Record<string, string>) =>
      Object.entries(values ?? {}).reduce((text, [key, value]) => text.replace('{' + key + '}', value), zh[key]),
  } as unknown as WorkflowPlatformPanelProps
}

it('creates a plugin-contributed task with its own input fields', async () => {
  const start = vi.fn().mockResolvedValue({})
  render(<WorkflowPlatformPanel {...props({ phase: 'ready', error: null, runs: [], definitions: [{
...
it('keeps run details as the default sheet and opens trajectory on demand', () => {
  render(<WorkflowPlatformPanel {...props({ phase: 'ready', error: null, definitions: [], runs: [{
