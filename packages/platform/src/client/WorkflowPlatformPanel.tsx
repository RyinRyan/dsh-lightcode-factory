...
import type { WorkflowPlatformSource } from 'lightcode-factory-backend/client'
import type { WorkflowRunStatus, WorkflowRunView } from 'lightcode-factory-backend/types'
import type { InjectFace, PropsLocale, PropsRuntime } from '@deepseek-ai/dsh-client-ui-slots'
import type { WorkflowPlatformKey } from './locales.ts'
import css from './WorkflowPlatformPanel.module.css'
import { WorkflowTrace } from './WorkflowTrace.tsx'
import { WorkflowRunOverview } from './WorkflowRunOverview.tsx'

export interface WorkflowPlatformInjected {
  readonly hooks: { readonly workflowSnapshot: WorkflowPlatformSource }
...
export type WorkflowPlatformPanelProps = PropsRuntime<'main'> & PropsLocale<'workflowPlatform'> & InjectFace<WorkflowPlatformInjected>
const LANES: readonly WorkflowRunStatus[] = ['queued', 'running', 'review', 'completed', 'cancelled', 'failed']

export function WorkflowPlatformIcon({ size, active }: Pick<PropsRuntime<'sidebar.panellist'>, 'size' | 'active'>) {
...
export function WorkflowPlatformPanel({ useWorkflowSnapshot, refresh, start, cancel, review, t }: WorkflowPlatformPanelProps) {
  const snapshot = useWorkflowSnapshot(value => value)
  ...
  return <main className={css.root}>
    ...
    {creating && <div className={css.taskModalBackdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) closeCreator() }}>
      <div ref={dialogRef} className={css.taskModal} role="dialog" aria-modal="true" aria-labelledby="workflow-create-title">
        <header className={css.taskModalHeader}>
          <span className={css.taskModalIcon}><WorkflowPlatformIcon size={24} active /></span>
    ...
    <section className={css.board}>{LANES.map(status => <section className={css.lane} key={status}><h2><i className={`${css.dot} ${css[status]}`} /><span>{t(statusTextKey(status))}</span><em>{grouped[status].length}</em></h2><div className={css.cards}>{grouped[status].map(run => <button className={css.card} key={run.id} type="button" aria-label={t('card.open', { name: run.name })} onClick={() => { setSelectedRunId(run.id) }}><strong>{run.name}</strong><span>{run.currentNodeId === undefined ? t(statusTextKey(run.status)) : run.nodes.find(node => node.id === run.currentNodeId)?.name}</span><time>{t('card.created', { time: new Date(run.createdAt).toLocaleString() })}</time></button>)}{grouped[status].length === 0 && <span className={css.empty}>{t('board.empty')}</span>}</div></section>)}</section>
  </main>
}

function RunDetail({ run, t, back, cancel, review }: { run: WorkflowRunView; t: WorkflowPlatformPanelProps['t']; back: () => void; cancel: () => void; review: (decision: 'complete' | 'cancel') => void }) {
  ...
  return <main className={`${css.root} ${css.runDetailRoot}`}><header className={`${css.header} ${css.runHeader}`}><div><div className={css.breadcrumb}><button type="button" onClick={back}>{t('detail.board')}</button><span>/</span><strong>{run.name}</strong></div><div className={css.runTitle}><h1>{run.name}</h1><span data-status={run.status}>{t(statusTextKey(run.status))}</span></div><p>Run ID　·　{run.id}</p></div><div className={css.actions}>{['queued', 'running'].includes(run.status) && <button className={css.danger} type="button" onClick={cancel}>□　{t('action.cancel')}</button>}{run.status === 'review' && <><button className={css.danger} type="button" onClick={() => { review('cancel') }}>□　{t('action.reject')}</button><button className={css.approve} type="button" onClick={() => { review('complete') }}>●　{t('action.approve')}</button></>}</div></header>
    ...
    {sheet === 'overview' ? <WorkflowRunOverview run={run} t={t} /> : <WorkflowTrace run={run} />}
  </main>
}

function statusTextKey(status: WorkflowRunStatus): WorkflowPlatformKey { return `status.${status}` }
