...
import type { WorkflowRegistration, WorkflowExecutionContext, WorkflowNodeContext } from './runtime-types.ts'
import type { KvTable } from '@deepseek-ai/dsh-storage-domain'
import { workflowPlatformDomain } from './spec.ts'
import type {
  ...
  WorkflowPlatformSnapshot, WorkflowRunEvent, WorkflowRunView,
} from './types.ts'

export type * from './types.ts'
export type * from './runtime-types.ts'
export { workflowPlatformDomain, workflowRunSchema } from './spec.ts'

declare module '@deepseek-ai/cordis' {
  interface Context { lightcodeFactoryBackend: LightcodeFactoryBackend }
}
...
export class LightcodeFactoryBackend extends TypertRemoteService {
  ...
  private table?: KvTable<string, WorkflowRunView>
  ...
  constructor(ctx: Context, config: Config) {
    super(ctx, 'lightcodeFactoryBackend', { namespace: 'workflowPlatform' })
    this.config = config as ResolvedConfig
  }

  /** Open durable storage and recover runs that survived a host restart. */
  protected async [Service.init](): Promise<void> {
    this.ctx.effect(() => this.ctx.typert.register({
      package: workflowRemote.package, face: 'host', schemas: [],
      invocations: workflowRemote.descriptors,
      model: { services: [], events: [], objects: [] },
    }), 'factory: wire contract')
    const domain = await this.ctx.storageDomain.open(workflowPlatformDomain)
    this.ctx.effect(() => async () => {
      ...
      await domain.close()
    }, 'lightcode factory: drain before closing storage')
    this.table = domain.table('runs')
    for (const [, run] of this.table.entries()) {
      if (run.status === 'running' || run.status === 'queued') await this.persist(this.appendEvent(this.clearCurrentNode({ ...run, status: 'failed', updatedAt: this.now(), error: 'Host restarted during execution' }), 'run.failed', 'DSH 重启导致执行中断'))
    }
    this.pump()
  }
  ...
  registerWorkflow(registration: WorkflowRegistration): () => Promise<void> {
    ...
    return async () => {
      ...
      for (const runId of ids) {
        if (['queued', 'running'].includes(this.requireRun(runId).status)) await this.cancel({ runId })
      }
  ...
  /** Read detached board data.
   * @returns Current registrations and persisted tasks, detached from storage.
   */
  snapshot(): WorkflowPlatformSnapshot {
    const runs = [...this.requireTable().entries()].map(([, run]) => structuredClone(run))
      .sort((left, right) => right.createdAt.localeCompare(left.createdAt))
    return { definitions: [...this.definitions.values()].map(value => structuredClone(this.metadata(value))), runs }
  }
  ...
  async cancel(request: WorkflowRunRequest): Promise<WorkflowRunView> {
    ...
    const cancelled = await this.update(runId, (run) => {
      if (!['queued', 'running', 'review'].includes(run.status)) throw new Error('Cannot cancel task in ' + run.status)
      const at = this.now()
  ...
  async review(request: WorkflowReviewRequest): Promise<WorkflowRunView> {
    ...
    if (decision === 'cancel') {
      if (this.requireRun(runId).status !== 'review') throw new Error('Task is not waiting for review')
  ...
  private pump(): void {
    while (!this.stopped && this.activeCount < this.config.maxConcurrentRuns && this.queuedIds.length > 0) {
      ...
      if (!registration || this.requireRun(runId).status !== 'queued') continue
      this.activeCount++
  ...
  private async execute(runId: string, registration: WorkflowRegistration): Promise<void> {
    ...
    try {
      ...
      const context: WorkflowExecutionContext = {
        runId, input: structuredClone(this.requireRun(runId).input ?? {}), signal,
  ...
  private update(id: string, change: (run: WorkflowRunView) => WorkflowRunView): Promise<WorkflowRunView> {
    ...
    const task = previous.catch((error: unknown) => {
      ...
      const next = { ...change(this.requireRun(id)), updatedAt: this.now() }
  ...
  private requireTable(): KvTable<string, WorkflowRunView> {
    if (this.table === undefined) throw new Error('workflow platform is not initialized')
    return this.table
  }

  private requireRun(runId: string): WorkflowRunView {
    const run = this.requireTable().get(runId)
    if (run === undefined) throw new Error(`workflow run '${runId}' does not exist`)
    return run
  }

  private async persist(run: WorkflowRunView): Promise<void> {
  ...
  private assertAcceptingTasks(): void {
    if (this.stopped) throw new Error('Backend is stopping')
  }
}

export default LightcodeFactoryBackend
