...
import * as Demo from '../src/index.ts'
...
describe('Factory Demo Loader composition', () => {
  it('runs real Agent tools and a real script, preserving tool pairs in the task detail', async () => {
    ...
    try {
      ...
      const modules = new Map<string, unknown>([['factory-backend', Backend], ['factory-demo', Demo]])
      ...
      expect(ctx.lightcodeFactoryBackend.snapshot().definitions.map(value => value.id)).toEqual(['morning-script-demo'])
      const run = await ctx.lightcodeFactoryBackend.start({ workflowId: 'morning-script-demo' })
      await vi.waitFor(() => {
      ...
      const task = ctx.lightcodeFactoryBackend.snapshot().runs[0]
