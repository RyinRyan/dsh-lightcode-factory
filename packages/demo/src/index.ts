...
import type { WorkflowNodeContext, WorkflowRegistration } from 'lightcode-factory-backend'

export const name = 'lightcode-workflow-demo'
export const inject = ['lightcodeFactoryBackend', 'agents', 'agentDefaultModel', 'sandbox', 'sandboxPolicy', 'subprocess']

/** Business execution limits owned by this workflow, not by the backend. */
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.effect(() => ctx.lightcodeFactoryBackend.registerWorkflow(workflow), 'register morning workflow')
}

async function generateScript(
  ctx: Context, config: Required<Config>, cwd: string, instruction: string, node: WorkflowNodeContext,
...
async function executeScript(
  ctx: Context, config: Required<Config>, cwd: string, code: string, node: WorkflowNodeContext,
