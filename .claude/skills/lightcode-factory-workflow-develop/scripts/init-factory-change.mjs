...
const validTypes = new Set(['backend', 'platform', 'cross-cutting'])

if (changeId === undefined || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(changeId) || !validTypes.has(changeType)) {
  console.error('用法：node init-factory-change.mjs <kebab-case-change-id> <变更名称> <backend|platform|cross-cutting> [factory-root]')
  process.exit(1)
}
