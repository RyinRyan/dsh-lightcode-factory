...
const backend = byName.get('lightcode-factory-backend')
for (const workflow of packages.filter(value => value.pkg.name?.startsWith('lightcode-workflow-'))) {
  if (workflow.pkg.dsh?.client !== undefined || workflow.pkg.exports?.['./client'] !== undefined) {
    errors.push(`${workflow.pkg.name}: 业务 workflow 应复用 Factory platform，不应声明 Browser Client`)
  }
  if (backend === undefined) {
    errors.push(`${workflow.pkg.name}: workspace 缺少 lightcode-factory-backend`)
  } else if (workflow.pkg.dependencies?.['lightcode-factory-backend'] !== backend.pkg.version) {
    errors.push(`${workflow.pkg.name}: lightcode-factory-backend 依赖必须与 workspace backend 版本 ${backend.pkg.version} 一致`)
  }
