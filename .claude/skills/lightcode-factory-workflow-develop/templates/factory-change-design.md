...
- Change ID: `{{CHANGE_ID}}`
- 变更类型: `{{CHANGE_TYPE}}`
- 影响组件: [填写：backend / platform / workflow / factory，可多选]
- 设计状态: 草稿
...
[填写：状态所有者、合法转换、mutation 顺序、取消、失败、卸载、停止、重启和晚到结果。纯 Platform 变更说明为何不影响状态。]

## 6. 持久化、Remote 与兼容性

[填写：历史数据、schema/domain 版本、wire schema、旧 Workflow/Browser 的兼容或迁移策略。]

## 7. Platform 与交互

[填写：页面信息架构、通用 renderer/回退、轨迹、可访问性和响应式影响；纯 Backend 变更说明 Browser 影响。]
...
- [ ] 组件选择正确，没有把业务特例放入 Backend/Platform。
- [ ] Backend 仍是 durable run 状态的唯一写入者。
- [ ] 类型、storage schema、wire schema 与调用方影响已覆盖。
- [ ] 历史数据、旧 Workflow 和 Browser 兼容策略明确。
- [ ] 取消、失败、卸载、停止、重启和竞态已分析。
- [ ] Platform 有通用回退、可访问性和无业务特例保证。
- [ ] 安全、凭据、日志/输出边界明确。
