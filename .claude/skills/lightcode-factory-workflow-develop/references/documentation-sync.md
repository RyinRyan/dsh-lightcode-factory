...
| `.design/changes/*.md` | Backend/Platform/跨组件设计、兼容、迁移、测试和实际结果 |
...
- `packages/backend/src`：更新 architecture；涉及开发规则时更新 core/backend/testing references；用户能力或配置变化更新 README。
- `packages/platform/src`：更新 architecture 与 platform-display/platform-development；用户可见能力变化更新 README。
...
完成前运行 `npm.cmd run audit:ai`。`--docs` 模式会读取本地 Git 变更：当 Backend、Platform、Workflow 或 Bundle 源码发生变化，却没有相应设计/架构/README 变更时失败。
