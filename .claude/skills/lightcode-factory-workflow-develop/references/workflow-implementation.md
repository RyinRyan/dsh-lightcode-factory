...
开始前必须已有审计通过的 `.design/workflows/<workflow-id>.md`。本文件给出稳定实现模式；准确类型名和字段仍以当前 Backend 公开 exports 为准。
...
```text
业务 Workflow --公开注册契约--> Factory Backend
Factory Platform --Remote/Snapshot--> Factory Backend
```

要求：

- 包名、版本、exports、files 和模块格式跟随当前 workspace；
- Backend 使用正式 package dependency，Cordis 按当前仓库约定声明；
- 不使用绝对 `file:` 依赖，不深层导入 Backend 源码；
- 默认没有 `dsh.client` 和 `./client`；
- 需要其他 Host 服务时显式声明 inject，并在设计中说明用途与缺失行为。

## 2. 注册模式

从当前 Backend 公开入口导入注册和节点上下文类型。实现应遵循以下形状，而不是照抄字段：
...
export function apply(ctx: Context): void {
  ctx.effect(
    () => ctx.lightcodeFactoryBackend.registerWorkflow(workflow),
...
注册放在 `ctx.effect`，使卸载可以注销 definition，并由 Backend 处理已接纳任务。不要直接写服务注册表。

## 3. 输入和业务校验

- 参数只承载允许持久化和展示的业务输入。
- 未知字段、必填和默认值由当前 Backend 契约处理；格式、范围、跨字段等业务校验在节点中完成。
- 如果当前 Backend 会先创建 run 再执行首节点，业务校验失败应被描述为可审计的 failed run，不能声称是“创建前拒绝”。
- credential 由 Host service、宿主设置或安全引用获取；参数最多保存非敏感资源 id。
- 产品必须在创建 run 前完成复杂校验时，这是 Backend 协议扩展，不在单个 Workflow 中伪造。
...
`node.log()` 和 `node.report()` 描述节点内部如何工作，不是 run 生命周期事件。kind 应复用当前 Platform 已识别的模型、工具、进程和上下文类别；新增领域事实使用稳定的 `<domain>.<fact>` 命名。
...
具体 kind 和限制值必须从当前 Platform/Backend 源码确认。

## 7. 状态和生命周期

Workflow 只发起节点工作。排队、开始、节点状态、评审、完成、取消、失败和重启恢复语义由 Backend 产生。节点作者只负责：
