# Backend / Platform 变更设计门禁

Backend、Platform 或跨组件变更必须在编码前创建 `.design/changes/<change-id>.md`。设计的目标是证明职责放置正确、状态只有一个所有者、公共契约可兼容演进，并明确文档和验证影响。
...
1. 用户可观察问题和成功标准是什么？
2. 为什么选择 Backend、Platform 或跨组件，而不是 Workflow？
3. 当前代码路径、类型/schema、状态和页面行为是什么？
4. 目标契约、数据流、状态所有者和失败/取消语义是什么？
5. 对已有 Workflow、持久化数据、Remote 和 UI 有何兼容影响？
...
```powershell
node .claude/skills/lightcode-factory-workflow-develop/scripts/audit-lightcode-workflow.mjs . --design .design/changes/<change-id>.md --docs
```
